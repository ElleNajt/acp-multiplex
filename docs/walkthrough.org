...
This is the key trick. Two frontends might both send a request with ~id: 1~. The agent would see two requests with the same id and get confused. So the proxy rewrites IDs:
...
And when the response comes back, restore the original:
...
The proxy keeps a monotonically increasing counter. Frontend A sends ~id:1~, proxy rewrites to ~id:7~, remembers "7 -> (Frontend A, original id 1)". Frontend B sends ~id:1~, proxy rewrites to ~id:8~. The agent sees 7 and 8 — no collision.
...
*** Reverse calls: primary only

The agent can also /send/ requests to the frontend — file reads (~fs/*~), terminal commands (~terminal/*~), permission requests (~session/requestPermission~). These go only to the primary frontend:
...
Notifications and responses pass through unchanged. Requests get ID-rewritten:
